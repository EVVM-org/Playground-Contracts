 ...
 */
export async function registerEvvmCrossChain(_args: string[], options: any) {
  console.log(
  ...
  let treasuryExternalStationAddress: `0x${string}` | undefined = options.treasuryExternalStationAddress;
  ...
  console.log(
    `${colors.blue}Setting Host Chain RPC...${colors.reset}\n`
  );
  let { rpcUrl: hostRpcUrl, chainId: hostChainId } = await getRPCUrlAndChainId(process.env.HOST_RPC_URL);
  console.log(
    `${colors.blue}Setting External Chain RPC...${colors.reset}\n`
  );
  let { rpcUrl: externalRpcUrl, chainId: externalChainId } = await getRPCUrlAndChainId(process.env.EXTERNAL_RPC_URL);
  ...
  const isSet = await callSetEvvmID(evvmAddress, evvmID, hostRpcUrl, walletName);
  ...
  if (!isSet) {
    ...
    console.log(`${colors.blue}Trying to set on External Chain...${colors.reset}\n`);
  }

  const isSetExternal = await callSetEvvmID(treasuryExternalStationAddress, evvmID, externalRpcUrl, walletName);

  if (!isSetExternal) {
    showError(
      `EVVM ID setting on External Chain failed.`,
      `\n${colors.yellow}You can try manually with:${colors.reset}\n${colors.blue}cast send ${treasuryExternalStationAddress} \\\n  --rpc-url ${externalRpcUrl} \\\n  "setEvvmID(uint256)" ${evvmID} \\\n  --account ${walletName}${colors.reset}`
    );
    return;
  }
